...
class sc(Scene):
    ...
    def construct(self):
        ...
        self.wait(1)
        code = Code("assets/code/Makefile", language="cpp", font_size=18, line_spacing=0.8)[2]
        self.play(Write(code))
        self.wait(3)

        self.ripple(code, range(0, 1), 1.5) # EXENAME
        self.wait(1)
        self.ripple(code, range(2, 5), -1) # CC/LIBS/SRCS
        self.wait(1)
        self.ripple(code, range(6, 7), 2.5) # TARGET : PREREQS
        self.wait(2)
        self.ripple(code, range(7, 8), 4) # SCRIPT
        self.wait(2)
